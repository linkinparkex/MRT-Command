...
using static Magpie.ScaleModelManager;
...
namespace Magpie {
	...
	public partial class MainWindow : Window {
		...
		//[*]
		private void ToggleMagWindow2(int hwnd, string effectsJson) {
			if (Settings.Default.AutoRestore) {
				StopWaitingForRestore();
				// 立即更新布局，因为窗口大小可能改变，如果接下来放大 Magpie 本身会立即退出
				UpdateLayout();
			...
			magWindow.Create2(hwnd, effectsJson);
		...
		private IntPtr WndProc(IntPtr hwnd, int msg, IntPtr wParam, IntPtr lParam, ref bool handled) {
			if (msg == NativeMethods.MAGPIE_WM_SHOWME) {
				...
				handled = true;
			} else if (msg == NativeMethods.MAGPIE_WM_DESTROY) {//[*]
				Logger.Info("收到 DESTROY 消息");
				Application_Closing();
				Closing -= Window_Closing;
				Close();
				handled = true;
			} else if (msg == NativeMethods.MAGPIE_WM_CREATE_NEW) {//[*]
				Logger.Info("收到 CREATE 消息");
				int hwndInt = 0;
				string effectsJson = null;

				try {
					string jsonStr = File.ReadAllText(@"C:\Windows\OEM\fsh\current_effect.json");
					JsonNode currentEffectJson = JsonObject.Parse(jsonStr);
					hwndInt = (int)currentEffectJson["hwnd"];
					if(hwndInt == 1231) {
						Thread.Sleep(3000);
						hwndInt = NativeMethods.GetForegroundWindow().ToInt32();
					}

					effectsJson = (string)currentEffectJson["effectsJson"];
				} catch (Exception e) {
					Logger.Error(e);
				}

				if (hwndInt != 0 && effectsJson != null && effectsJson != "") {
					ToggleMagWindow2(hwndInt, effectsJson);
				}
		...
		public class CurrentEffect {
			...
			public string effectsJson { get; set; } = null;
		}
