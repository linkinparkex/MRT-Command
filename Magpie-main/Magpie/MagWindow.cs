...
namespace Magpie {
	...
	internal class MagWindow : IDisposable {
			...
			magThread = new Thread(() => {
				...
				while (magWindowParams.cmd != MagWindowCmd.Exit) {
					...
					if (cmd == MagWindowCmd.SetLogLevel) {
						...
						uint flags = (false ? (uint)FlagMasks.NoCursor : 0) | //不绘制光标
									(true ? (uint)FlagMasks.AdjustCursorSpeed : 0) | //缩放时调整光标速度
									(false ? (uint)FlagMasks.SaveEffectSources : 0) | //解析效果时保存源代码
									(false ? (uint)FlagMasks.DisableLowLatency : 0) |
									(false ? (uint)FlagMasks.BreakpointMode : 0) | //断点模式
									(false ? (uint)FlagMasks.DisableWindowResizing : 0) | //缩放时禁用窗口大小天正
									(false ? (uint)FlagMasks.DisableDirectFlip : 0) | //禁用DirectFlip
									(false ? (uint)FlagMasks.Is3DMode : 0) | //是否3d游戏模式
									(true ? (uint)FlagMasks.CropTitleBarOfUWP : 0) | //裁剪UWP窗口的标题栏
									(false ? (uint)FlagMasks.DisableEffectCache : 0) | //禁用效果缓存
									(false ? (uint)FlagMasks.SimulateExclusiveFullscreen : 0) | //模拟独占全屏
									(false ? (uint)FlagMasks.WarningsAreErrors : 0) |
									(true ? 0 : (uint)FlagMasks.DisableVSync) | //开启垂直同步
									(false ? (uint)FlagMasks.ShowFPS : 0); // 显示fps
		...
		//[*]
		public void Create2(int hwnd, string effectsJson) {
			if (Running) {
				Logger.Info("已存在全屏窗口，取消进入全屏");
				return;
			}

			IntPtr hwndSrc = new IntPtr(hwnd);
			if (!NativeMethods.IsWindow(hwndSrc)
				|| !NativeMethods.IsWindowVisible(hwndSrc)
				|| NativeMethods.GetWindowShowCmd(hwndSrc) != NativeMethods.SW_NORMAL
			) {
